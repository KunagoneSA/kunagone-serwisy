    ...
    try {
      if (isEdit) {
        const { error: updateError } = await supabase
          .from('assets')
          .update({ name, identifier, type, notes: notes || null, metadata: metadataObj })
          .eq('id', asset.id)
        ...
        const { error: insertError } = await supabase
          .from('assets')
          .insert({ name, identifier, type, notes: notes || null, metadata: metadataObj, created_by: user?.id ?? null })
